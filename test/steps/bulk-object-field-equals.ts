...
import { Step } from '../../src/steps/bulk-object-field-equals';

chai.use(sinonChai);

describe('BulkObjectFieldEqualsStep', () => {
  ...
  beforeEach(() => {
    clientWrapperStub.bulkFindObjectsById = sinon.stub();
  ...
  it('should respond with pass if API client resolves expected data', async () => {
    ...
    const expectedUser: any = [{ Id: 'someid1', someField: 'Expected Value' }, { Id: 'someid2', someField: 'Expected Value' }];
    clientWrapperStub.bulkFindObjectsById.resolves(expectedUser);
    ...
    expect(clientWrapperStub.bulkFindObjectsById).to.have.been.calledWith(expectations.objName, ['someid1', 'someid2']);
    expect(response.getOutcome()).to.equal(RunStepResponse.Outcome.PASSED);
  });

  it('should respond with error when invalid operator was passed', async () => {
    // Stub a response that matches expectations.
    const expectedUser: any = [{ someField: 'Expected Value' }, { someField: 'Expected Value' }];
    clientWrapperStub.bulkFindObjectsById.resolves(expectedUser);
    ...
    const expectations: any = {
      ...
      expectedValue: expectedUser[0].someField,
    };
    protoStep.setData(Struct.fromJavaScript(expectations));
  ...
  it('should respond with error when actual and expected values have different types and compared', async () => {
    // Stub a response that matches expectations.
    const expectedUser: any = [{ someField: 'Expected Value', numericField: 5000 }, { someField: 'Expected Value', numericField: 5000 }];
    clientWrapperStub.bulkFindObjectsById.resolves(expectedUser);
  ...
  it('should respond with fail if API client resolves unexpected data', async () => {
    ...
    const expectedUser: any = [{ Id: 'someid1', someField: 'Expected Value', numericField: 5000 }, { Id: 'someid2', someField: 'Expected Value', numericField: 5000 }];
    clientWrapperStub.bulkFindObjectsById.resolves(expectedUser);
  ...
  it('should respond with fail if API client resolves no results', async () => {
    ...
    const expectedUser: any = [{ Id: 'someid1', someField: 'Expected Value', numericField: 5000 }, { Id: 'someid2', someField: 'Expected Value', numericField: 5000 }];
    clientWrapperStub.bulkFindObjectsById.resolves([]);
  ...
  it('should respond with error if API client returns error', async () => {
    ...
    clientWrapperStub.bulkFindObjectsById.throws(error);
